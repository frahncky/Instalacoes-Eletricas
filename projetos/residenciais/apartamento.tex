...
\begin{frame}[shrink=8]{Apartamento — mínimos por ambiente}
\scriptsize
...
\begin{frame}[shrink=6]{Apartamento — circuitos A1 a A6}
\scriptsize
...
\begin{frame}{Apartamento — circuitos A7 a A13}
\scriptsize
\begin{tabularx}{\textwidth}{c p{2.5cm} c c c c c X}
\toprule
C & Equipamento & $P$ & $I_b$ & Cu & DJ & Fase & Proteção e nota\\
\midrule
...
\end{align*}
Total equivalente: \SI{25.78}{kW}. Proteção de 63 A é a premissa inicial da unidade. Nesta ilustração, os VA de previsão foram tomados numericamente como W; a declaração real usa potências ativas nominais. Verificar cenários de demanda, pois os três aparelhos de ar-condicionado estão na fase A.
\end{caixaProjeto}
\column{0.49\textwidth}
...
\node[b,font=\tiny](l)at(-3.9,0.9){A1--A6\\luz/TUG + RCBO};\node[b,font=\tiny](ch)at(-1.3,0.9){A7--A8\\chuveiros};\node[b,font=\tiny](co)at(1.3,0.9){A9--A10\\forno/lavadora};\node[b,font=\tiny](ac)at(3.9,0.9){A11--A13\\climatização};
\node[b,draw=corNorma,fill=corNorma!7](bep)at(4.8,2.9){BEP do edifício\\PE/equipot.};
\draw[-{Stealth},thick,corPrim](cm)--(pr)--(qd);\foreach \x in {l,ch,co,ac}{\draw[-{Stealth},thick](qd)--(\x);}\draw[corNorma,thick](pr)--(bep);\draw[corNorma,thick](qd)--(bep);\draw[corNorma,thick](bep.east)--(5.95,2.9)--(5.95,1.55)node[ground]{};
