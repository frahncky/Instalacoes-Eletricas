...
\coordinate (retornoDireita) at ([yshift=-0.45cm]i.south);
\coordinate (retornoEsquerda) at ([xshift=-0.35cm,yshift=-0.45cm]g.south west);
\draw[-{Stealth},thick,corAccent]
  (i.south)--(retornoDireita)--(retornoEsquerda)--
  ([xshift=-0.35cm]a.west)--(a.west);
\end{tikzpicture}
\vspace{2mm}
\begin{caixaAt}
