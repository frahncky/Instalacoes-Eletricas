...
\begin{frame}{Mapa normativo — documentos-base em agosto de 2026}
\scriptsize
\begin{tabularx}{\textwidth}{>{\bfseries}p{2.75cm}p{2.8cm}X}
\toprule
Documento & Situação usada no material & Aplicação \\
\midrule
ABNT NBR 5410:2004 & Referência vigente adotada para BT; a NT.00001.EQTL Rev.09 a cita expressamente. & Choque, seleção e instalação de componentes, condutores, circuitos, DR, aterramento, verificação e manutenção. \\
ABNT NBR 14039:2021 & Referência vigente adotada para MT. & Instalações com tensão nominal de 1,0 kV a 36,2 kV, quando a solução de atendimento ou a instalação interna sair da BT. \\
ABNT NBR 5419-1:2026 & Parte 1 publicada em 2026. & Requisitos para a determinação da proteção contra descargas atmosféricas; registrar separadamente a edição de cada parte usada. \\
ABNT NBR 17019:2022 & Referência para alimentação de veículos elétricos. & Instalações fixas para carga e descarga de energia de VE, em conjunto com a NT.00042.EQTL Rev.04 na área da Equatorial. \\
\bottomrule
\end{tabularx}
\begin{caixaAt}
Norma de instalação interna, norma da distribuidora e requisito legal têm escopos diferentes. O projeto deve atender simultaneamente aos documentos aplicáveis, sem usar um para substituir o outro.
\end{caixaAt}
\end{frame}

\begin{frame}[shrink=5]{Documentos complementares por tipo de edificação}
\scriptsize
\begin{tabularx}{\textwidth}{>{\bfseries}p{3.15cm}p{2.55cm}X}
\toprule
Documento & Situação usada & Aplicação \\
\midrule
ABNT NBR 13534:2008 & Conferir edição vigente no início do projeto. & Requisitos específicos para estabelecimentos assistenciais de saúde; não se aplica automaticamente a escritórios ou salas de atendimento sem uso médico. \\
ABNT NBR 13570:2021 & Citada pela NT.00001.EQTL Rev.09. & Instalações em locais de afluência de público, quando caracterizados no caso real. \\
Série ABNT NBR IEC 61439 & Registrar parte e edição aplicáveis. & Conjuntos de manobra e comando de BT; o quadro deve ser especificado como conjunto, não como soma informal de componentes. \\
NR-10 atual & Redação consolidada pela Portaria SEPRT 915/2019, vigente até 31/05/2027. & Segurança em instalações e serviços com eletricidade. \\
NR-10 futura & Portaria MTE 737/2026, vigência em 01/06/2027. & Nova redação já publicada; não deve ser apresentada como vigente antes da data definida pelo MTE. \\
\bottomrule
\end{tabularx}
\end{frame}

\begin{frame}[shrink=5]{Equatorial Energia — referências atuais para o Maranhão}
\scriptsize
...
NT.00002.EQTL & Rev.10 — 29/05/2025 & Fornecimento de energia em média tensão, quando a solução de conexão da edificação não permanecer em BT. \\
NT.00004.EQTL & Rev.08 — 31/12/2025 & Empreendimentos/edificações de múltiplas unidades consumidoras; Anexo I de queda de tensão publicado em 21/05/2026. \\
NT.00020.EQTL & Rev.06 — 31/12/2025 & Conexão de micro e minigeração distribuída; formulários dos Grupos A e B atualizados em 27/05/2026. \\
...
\begin{frame}[shrink=4]{Equatorial Maranhão: limites que afetam o projeto residencial}
\small
\begin{columns}[T,onlytextwidth]
...
\end{columns}
\begin{caixaObs}
O projeto residencial revisado fecha em \textbf{33,96 kW na hipótese didática declarada} e, por isso, usa a faixa trifásica de 24,1 a 38 kW, com proteção de entrada de 63 A. O enquadramento vem depois do levantamento, não antes.
\end{caixaObs}
\end{frame}
...
\begin{frame}{NR-10: texto vigente e nova redação já publicada}
\small
\begin{columns}[T,onlytextwidth]
\column{0.52\textwidth}
\begin{caixaNorma}[title=Portaria MTE nº 737/2026]
A Portaria é de 29/05/2026 e fixa a vigência da nova redação da NR-10 em \textbf{01/06/2027}. Até 31/05/2027 permanece vigente a redação consolidada pela Portaria SEPRT nº 915/2019.
\end{caixaNorma}
\begin{itemize}
...
\coordinate (retornoDireita) at ([yshift=-0.48cm]i.south);
\coordinate (retornoEsquerda) at ([xshift=-0.38cm,yshift=-0.48cm]g.south west);
\draw[-{Stealth},thick,corAccent]
  (i.south)--(retornoDireita)--
  node[midway,below,font=\tiny]{retroalimentar o projeto}
  (retornoEsquerda)--([xshift=-0.38cm]a.west)--(a.west);
\end{tikzpicture}
\end{frame}
